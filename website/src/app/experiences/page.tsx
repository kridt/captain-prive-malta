      ...
      <section className="py-28 bg-sand-50">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {PACKAGES.map((pkg, i) => (
              <AnimatedReveal key={pkg.slug} delay={i * 80}>
                <Link
                  href={`/experiences/${pkg.slug}`}
                  className="card-premium group flex flex-col overflow-hidden rounded-2xl bg-white shadow-sm border border-sand-200/50 h-full"
                >
                  {/* Image */}
