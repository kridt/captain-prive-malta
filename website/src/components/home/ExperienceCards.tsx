    ...
    <section className="relative py-28 bg-white">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-20 grid gap-8 md:grid-cols-3">
          {featured.map((pkg, i) => (
            <AnimatedReveal key={pkg.slug} delay={i * 150} variant="fadeUp">
              <motion.div
                whileHover={{ y: -6 }}
              ...
              >
                <Link
                  ...
                  className="group block overflow-hidden rounded-2xl bg-sand-50 shadow-sm border border-sand-200/50 transition-shadow duration-500 hover:shadow-xl"
                >
                  {/* Image placeholder */}
                  <div className="relative aspect-[4/3] overflow-hidden bg-gradient-to-br from-navy-400 to-sea-500">
                    <div className="absolute inset-0 bg-gradient-to-t from-navy-900/70 via-transparent to-transparent" />
                    <div className="absolute bottom-4 left-4">
                      <span className="inline-flex items-center gap-1 rounded-full bg-black/30 backdrop-blur-sm border border-white/10 px-3.5 py-1.5 text-xs font-semibold text-white">
